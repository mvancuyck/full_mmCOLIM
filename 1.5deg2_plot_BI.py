...
def bI_from_autospec_and_cat(n_slices, field_size, ksimu, z_list, dz_list,
                       allpoints = False, recompute=False, nfields=70):
    
    list_I =  np.zeros((nfields,len(z_list), len(dz_list), len(line_list[:8])))
    list_Gal= np.zeros((nfields,len(z_list), len(dz_list), len(line_list[:8]),17))
    list_J =  np.zeros((nfields,len(z_list), len(dz_list), len(line_list[:8]),17))
    list_K =  np.zeros((nfields,len(z_list), len(dz_list), len(line_list[:8]),17))
    list_k =  np.zeros((nfields,len(z_list), len(dz_list), len(line_list[:8]),17))
        ...
        for zi, z in enumerate(z_list):
            for dzi, dz in enumerate(dz_list):
                for j, (J, rest_freq) in enumerate(zip(line_list[:8], rest_freq_list[:8])):
                    ...
                    #-----------------
                    K = ((dict["k"].to(u.rad**-1) * 2 * np.pi  /  dict["Dc"]).value)[:17]

                    list_K[nfield, zi, dzi, j,:K.shape[0]] = K
                    list_k[nfield, zi, dzi, j,:] = (dict["k"].to(u.arcmin**-1).value)[:17]

                    list_I[nfield, zi, dzi, j] = dict["I"][0].value
                    G = (dict["pk_gal"][0].value- dict["gal_shot"][0]       )[:17] 
                    JJ = (dict["pk_J"][0].value - dict["LIM_shot"][0].value )[:17] 
                    list_Gal[nfield, zi, dzi, j,:] = G
                    list_J[nfield, zi, dzi, j,:] = JJ

    b_list = np.zeros((nfields,len(z_list), len(dz_list), len(line_list[:8]), 2, 2))
        ...
        for zi, z in enumerate(z_list):
            for dzi, dz in enumerate(dz_list):
                for j, (J, rest_freq) in enumerate(zip(line_list[:8], rest_freq_list[:8])):
                    ...
                    W = np.where(Gal>0)
                    bgal,dbgal= fit_b_gal(     dict["k_angular"], dict["pk_matter_2d"], k[W], Gal[W], sigma_Gal[W]) 
                    W = np.where(JJ>0)
                    bX, dbX   = fit_b_eff_auto(dict["k_angular"], dict["pk_matter_2d"], k[W], JJ[W],  sigma_JJ[W], 
                                               I, sigma_I )
                    ...
                    if(False and nfield==0 and J=='CO32'):
                        ...
                        axj.errorbar( dict["k"][:-1], dict["pk_J"][0][:-1].value, yerr=np.std(list_J[:, zi, dzi, j, :], axis=0),
                                   c='darkgray', ls="--", lw=lw)
                        ...
                        axj.errorbar( k, JJ, sigma_JJ,fmt='o', color='r', ecolor='r',markersize=mk, label='points used in fit', zorder=5)
                        ...
                        axg.loglog( dict["k"][:-1], dict["pk_gal"][0][:-1].value, c='darkgray', ls="--",)
                        axg.errorbar( dict["k"][:-1], dict["pk_gal"][0][:-1].value, yerr=np.std(list_Gal[:, zi, dzi, j, :], axis=0),  c='darkgray', ls="--",)
...
def bI_crosspec(n_slices, field_size, ksimu, z_list, dz_list, b_list,
                       allpoints = False, recompute=False, dtype='', nfields=70):
    
    bI_mes = np.zeros((nfields,len(z_list), len(z_list), len(line_list[:8])))
    list_k = np.zeros((nfields,len(z_list), len(z_list), len(line_list[:8]),17))
    bI_mesCI = np.zeros((nfields,len(z_list), len(z_list),))
    bI_mesCO76 = np.zeros((nfields,len(z_list), len(z_list),))
        ...
        for zi, z in enumerate(z_list):
            for dzi, dz in enumerate(dz_list):
                for j, (J, rest_freq) in enumerate(zip(line_list[:8], rest_freq_list[:8])):
                    ...
                    #-----------------
                    K = dict["k"].to(u.arcmin**-1)[:17]
                    w = np.where( K.value <= 0.1)
                    k_a = w[0][0]; k_e =  w[0][-1]
                    if(k_e <= 1):  k_e = 2
                    if(allpoints): born = [0,-1]
                    else: born = [k_a,k_e+1]
                    ...
                    if(False and nfield==0 and J=='CO32'):
                        ...
                        plt.loglog(dict["k"].to(u.arcmin**-1).value, dict["pk_J-gal"][0], 'ob' )
...
def bI_from_tinker(n_slices, field_size, ksimu, z_list, dz_list,
    ...
    list_bI =  np.zeros((70,len(z_list), len(dz_list), len(line_list[:8])))
    list_b =  np.zeros((70,len(z_list), len(dz_list), len(line_list[:8])))

    for nfield in range(70):
        for zi, z in enumerate(z_list):
            for dzi, dz in enumerate(dz_list):
                for j, (J, rest_freq) in enumerate(zip(line_list[:8], rest_freq_list[:8])):
...
def b_I(n_slices, z_list, dz_list, ref = 'CO32',  dtype='all_lines', recompute=False):
    ...
    b_list, I_list = bI_from_autospec_and_cat(n_slices,1.5,1, z_list, dz_list )
    bI_tinker, b_tinker = bI_from_tinker(n_slices,1.5,1, z_list, dz_list)
    bI_mes, bI_mesCO76, bI_mesCI = bI_crosspec(n_slices,1.5,1, z_list, dz_list, b_list,dtype = '_with_interlopers')
    idz=0
    ...
    #---------------------------
    for extension in ("png", "pdf"): plt.savefig(f"bI_CO_1.5deg2_nslice{nslice}_dz{dz_list[0]}.{extension}", transparent=True)
    plt.show()
    
def rho(n_slices, z_list, dz_list, ref = 'CO32',  alphaco=3.6, dtype='all_lines', recompute=False):
    ...
    for j, (line, rest_freq) in enumerate(zip(line_list_fancy[:6], rest_freq_list[:6])):
        ...
        dict_of_excitation = pickle.load( open(f'dict_dir/SLED_mes_1.5deg2_dtype_with_interlopers.p', 'rb'))
        ...
        ax.errorbar(z_list, np.mean(Btinker/excitation_MES*rhoh2, axis=0), yerr=np.std(Btinker/excitation_ref*EXCITATION_ASSUMED*rhoh2, axis=0),  
                    linestyle="dashdot", color='k', ecolor='k', marker="p",markersize = mks,lw=lw,  mfc='none', mec='k')
        #-------------------------
        dict = pickle.load( open('rho_1.5deg2.p', 'rb'))
        RHO = np.transpose(dict['rho_1.5deg2'])
        ...
        ax.errorbar( z_list, np.mean(RHO, axis=0), linestyle="--", color='lightgreen', ecolor='lightgreen', markersize = mks,lw=lw,  mfc='none', mec='k')
        ax.fill_between( z_list, 
                        np.mean(RHO, axis=0)-np.std(RHO, axis=0), 
                        np.mean(RHO, axis=0)+np.std(RHO, axis=0),
    ...
    fig.tight_layout(); fig.subplots_adjust(hspace=.0)
    #---------------------------
    for extension in ("png", "pdf"): plt.savefig(f"excitation_rho_1.5deg2_nslice{nslice}_dz{dz_list[0]}.{extension}", transparent=True)
    plt.show()
    ...
    for j, (line, rest_freq) in enumerate(zip(line_list_fancy[:8], rest_freq_list[:8])):
        ...
        if(j==7): ax.legend(handles = patchs, loc='center left', bbox_to_anchor=(-0.4, -0.7), fontsize=7, frameon=False)
        print('')
        print(line, 100*(np.mean(bI_tinker[:, :, idz, j], axis=0)/np.mean(b_list[:, :, idz, j,0,0]*I_list[:,:,idz,j], axis=0)))
        print('')
...
def rho_excitation(n_slices, z_list, dz_list, ref = 'CO32',  alphaco=3.6, dtype='all_lines', recompute=False):
    ...
    for j, (line, rest_freq) in enumerate(zip(line_list_fancy[:6], rest_freq_list[:6])):
        ...
        nu_obs = 115.27120180 / (1+z_list)
        Lprim = 3.11e10/(nu_obs*(1+z_list))**3
        rhoh2 = excitation_ref/B * rhoL *alphaco*Lprim

        dict = pickle.load( open('rho_9deg2.p', 'rb'))
        rhocat = np.transpose(dict['rho_9deg2'])

        ax.errorbar( z_list, np.mean(rhocat/EXCITATION_ASSUMED, axis=0), linestyle="--", color='grey', ecolor='grey', markersize = mks,lw=lw,  mfc='none', mec='k')
        ax.fill_between(z_list, 
                        np.mean(rhocat/EXCITATION_ASSUMED, axis=0)-np.std(rhocat/EXCITATION_ASSUMED, axis=0), 
                        np.mean(rhocat/EXCITATION_ASSUMED, axis=0)+np.std(rhocat/EXCITATION_ASSUMED, axis=0),
                        color='grey', alpha=0.3)

        ax.errorbar(    z_list, np.mean(B*rhoh2, axis=0),  linestyle='solid', color=c, ecolor=c, lw=lw) 
        ax.fill_between(z_list, np.mean(B*rhoh2, axis=0)-np.std(B*rhoh2, axis=0), np.mean(B*rhoh2, axis=0)+np.std(B*rhoh2, axis=0), alpha = 0.2, color=c)

        ax.errorbar(    z_list, np.mean(Btinker*rhoh2, axis=0), linestyle="dashdot", color='k', ecolor='k', marker="p",markersize = mks,lw=lw,  mfc='none', mec='k')
        ax.set_ylabel("$\\rm \\rho_{H2} \\times R^{1-3} [M_{\\odot}.Mpc^{-3}]$"+f'\n from {line}')#+'$\\rm [Jy/sr]$')
        ax.set_xlim(0.4, np.max(z_list)+0.1)
        ax.set_yscale("log")
        if(j<4): ax.tick_params(axis = "x", which='major', tickdir = "inout", bottom = True, top = True, labeltop=False,labelbottom=False)
        else:
    ...
    fig.tight_layout(); fig.subplots_adjust(hspace=.0)
    #---------------------------
    for extension in ("png", "pdf"): plt.savefig(f"rho_times_excitation_nslice{nslice}_dz{dz_list[0]}.{extension}", transparent=True)
    plt.show()
