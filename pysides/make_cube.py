...
def save_cube(output_path, run_name, component_name, cube_type, cube_prop_dict, unit, input_cat, dz,cube=None, 
              recompute = False): 
    ...
    if(not os.path.isfile(filename) or recompute):
        ...
        hdr["BUNIT"] = unit
        hdr['COMMENT'] = f'dz = {dz}, Input catalog = {input_cat}, '
...
def make_fir_lines_cube(cat, params_sides, params, cube_prop_dict):
    ...
    for line_name, line_nu in zip(params_sides['fir_lines_list'], params_sides['fir_lines_nu']):
        print('Compute channel locations and flux densities of ['+line_name+'] lines...')
        Snu, channels = line_channel_flux_densities(line_name, line_nu, cat, cube_prop_dict)
