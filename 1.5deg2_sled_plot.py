...
def plot_sled_fig(nslice, z_list, dz_list, recompute_sleds, toembed=False):
    ...
    SLED_mes = co_sled_from_nsubfields(nslice, 2, z_list, dz_list, 1.5, 0.1, toembed=toembed)


    dict = {'SLED_mes':SLED_mes}
    pickle.dump(dict, open(f'dict_dir/SLED_mes_1.5deg2_dtype_with_interlopers.p', 'wb'))
    ...
    for idz, dz in enumerate(dz_list):
        ...
        for zi, (z, c, shift) in enumerate(zip(z_list,  cm.cividis(np.linspace(0.,1,len(z_list))),  (-0.2, -0.1, 0, 0.1, 0.2, 0.3))): #(-0.3, -0.2, -0.1, 0, 0.1, 0.2, 0.3, 0.4),
            ...
            ax.errorbar(J_list+shift, np.mean(SLED_mes[:, zi, idz, 1:], axis=0), yerr=np.std(SLED_mes[:, zi, idz, 1:], axis=0), 
                        fmt='o', color='k', ecolor='k', lw=1, markersize=2)
            print('')
            print('')
        ...
        for extension in ("png", "pdf"): plt.savefig(f"sled_dz{dz}_nslice{nslice}_1.5.{extension}", transparent=True)

        plt.show()

def co_sled_from_nsubfields(nslice, i_ref, z_list, dz_list, field_size, klim,
                            interlopers = None, allpoints=False, dtype='_with_interlopers', toembed=False, nfields=70):
    
    #---------------------------
    SLED_mes = np.zeros((nfields, len(z_list), len(dz_list), 9)) #12 subfields, redshift, redshift width, ref+8lines
